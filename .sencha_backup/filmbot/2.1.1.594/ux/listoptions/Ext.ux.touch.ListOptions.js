...
Ext.define('Ext.ux.touch.ListOptions',
{
    ...
    * @param {Object} e
    */
    onItemSwipe: function (dataView, index, target, record, eventOblect) {
        // check we're over the 'swipethreshold'
        if (this.revealAllowed(eventOblect.direction, eventOblect.distance)) {

            // set the direction of the reveal
            this.itemSwipeRevealDirection(eventOblect.direction);
    ...
    */
    createOptionsMenu: function (listItemEl) {
		...
		var listQueryItens = this.activeListOptions.select('.' + this.getMenuOptionSelector()).elements;
		for(var i=0; i< listQueryItens.length; i++)
		{
			Ext.get(listQueryItens[i]).on({
